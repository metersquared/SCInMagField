function matrix=bcsRectangularCell(in)
    Nx=in.Nx;
    Ny=in.Ny;
    chemPot=in.chemPot;
    hopInt=in.hopInt;
    gapArray=in.gapArray;
    b=in.b;
...
%-------------------------------------------    
    ...
    tRightBoundaryPhase=exp(phase*[1:Ny]*(1));
    tLeftBoundaryPhase=exp(-phase*[1:Ny]*(1));
...
%-------------------------------------------
    ...
    tDown=diag(D,Nx)+diag(C,Nx-totState);
    tUp=diag(C,totState-Nx)+diag(D,-Nx);
...
%    This is implementation of the y direction Landau gauge.
    ...
    tDownArray(:,Nx*Ny-Nx+1:Nx*Ny)=diag(exp(phase*(1:(Nx))*(Ny)));
...
%-------------------------------------------
    ...
    matrix=[laplaceMatrix gapMatrix; ctranspose(gapMatrix) -conj(laplaceMatrix)];
end
